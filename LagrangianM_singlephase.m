...
% ZIn  = find(abs(ZI) < 1e-6);   % or exactly ==0 if you prefer
ZIn = [];
nzi  = numel(ZIn);
...
Rdiag = [ ...
     ...
     0.01^2 * ones(4*nl, 1) ];                 % flows
R = spdiags(Rdiag, 0, numel(Rdiag), numel(Rdiag));
W = inv(R);
% Convert z to row (just to match code that uses z(:).'):
z = z(:).';
...
while flag
    ...
    Vc = x(nb+1:2*nb) .* exp(1j*x(1:nb));  % V_i
    [H_full, Sf, St] = makeJaco(x, Ybus, Yf, Yt, nb, nl, fbus, tbus, Vc);
    % remove ref angle column
    H_full(:, ref) = [];

    % remove zero‐injection rows from H_full
    C = H_full([nb+ZIn; 2*nb+ZIn], :);
    H = H_full;
    H([nb+ZIn; 2*nb+ZIn], :) = [];
    ...
    % remove zero‐injection entries
    cx = hx_full([nb+ZIn; 2*nb+ZIn]);
    hx = hx_full;
    hx([nb+ZIn; 2*nb+ZIn]) = [];
    ...
    if max(abs(dx)) < eps
        ...
        x0 = x0 + dx;       % update states
        xl = xl + dxl;      % update multipliers
        x(nstate) = x0;     % fill back
        k = k + 1;
        if k >= 20
            % did not converge
...
%% === 8) Build Parameter Jacobian w.r.t. r & x for each line ===
    ...
    V_est = x0(nb : 2*nb - 1);
    % --- assume V_est and theta_est are defined as before ---
    Hp_full = zeros(3*nb + 4*nl, 2*nl);
    ...
    for kk = 1:nl
        ...
        % --- CORRECTED LINE ---
        dPi_db = Vi*Vj*sind; % Was -Vi*Vj*sind

        % Q_i
        dQi_dg = Vi*Vj*sind;
        % --- CORRECTED LINE ---
        dQi_db = -Vi^2 + Vi*Vj*cosd; % Was -Vi^2 - Vi*Vj*cosd

        % Partial derivatives of injections w.r.t g_ij, b_ij for bus j
        dPj_dg = Vj^2 - Vi*Vj*cosd;
        % --- CORRECTED LINE ---
        dPj_db = -Vi*Vj*sind; % Was +Vi*Vj*sind

        % Q_j
        dQj_dg = -Vi*Vj*sind;
        % --- CORRECTED LINE ---
        dQj_db = -Vj^2 + Vi*Vj*cosd; % Was -Vj^2 - Vi*Vj*cosd
        ...
        % place into Hp_full
        Hp_full(nb + i,   2*kk-1) = dPi_dr; Hp_full(nb + i,   2*kk) = dPi_dx;
        Hp_full(nb + j,   2*kk-1) = dPj_dr; Hp_full(nb + j,   2*kk) = dPj_dx;
        Hp_full(2*nb + i, 2*kk-1) = dQi_dr; Hp_full(2*nb + i, 2*kk) = dQi_dx;
        Hp_full(2*nb + j, 2*kk-1) = dQj_dr; Hp_full(2*nb + j, 2*kk) = dQj_dx;
    
        % Pf (i->j) => same as P_i
        Hp_full(3*nb + kk,         2*kk-1) = dPi_dr;
...
%% remove zero-injection rows from Hp_full
Cp = Hp_full([nb+ZIn; 2*nb+ZIn], :);
Hp = Hp_full;
Hp([nb+ZIn; 2*nb+ZIn], :) = [];
...
Pf_f = real(Sf); Qf_f = imag(Sf);
Pt_f = real(St); Qt_f = imag(St);
...
final_resid = z(:) - hx_full(:);  % unnormalized

% Normalized residual:
Rfull_inv = W;  % Weighted by W
