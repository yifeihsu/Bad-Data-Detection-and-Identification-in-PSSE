...
% For demonstration, we'll assume you want some default small variance for all valid data.
% Or you might have a separate variance array that matches each measurement type.
% We'll do something simple:
% Rdiag_all = 0.0001^2 * ones(nMeasFull,1);  % default
R_variances = zeros(length(z_in), 1);

for i = 1:length(z_in)
    if i <= nb
        R_variances(i) = (0.01)^2;   % big variance => less weighting
    else
        R_variances(i) = (0.002)^2;
    end
end
